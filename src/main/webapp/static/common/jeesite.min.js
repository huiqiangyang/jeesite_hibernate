...
$.validator.setDefaults({submitHandler: function (a) {
    loading("正在提交，请稍等...");
    a.submit()
...
function tableSort(f) {
    var c = {orderBy: "#orderBy", contentTable: "#contentTable", sortClass: "sort", callBack: $.noop};
    var a = $.extend({}, c, f);
    var b = $(a.orderBy), e = $(a.contentTable + " th." + a.sortClass), d = b.val().split(" ");
    e.each(function () {
        if ($(this).hasClass(d[0])) {
            d[1] = d[1] && d[1].toUpperCase() == "DESC" ? "down" : "up";
            $(this).html($(this).html() + ' <i class="icon icon-arrow-' + d[1] + '"></i>')
        }
    });
    e.click(function () {
