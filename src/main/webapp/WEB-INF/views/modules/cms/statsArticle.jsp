...
<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/stats/article" method="post"
    ...
    <div>
        <label>归属栏目：</label><tags:treeselect id="category" name="categoryId" value="${paramMap.categoryId}"
                                             labelName="categoryName" labelValue="${paramMap.categoryName}"
                                             title="栏目" url="/cms/category/treeData" module="article"
                                             cssClass="input-small" allowClear="true"/>
        <label>归属机构：</label><tags:treeselect id="office" name="officeId" value="${paramMap.officeId}"
                                             labelName="officeName" labelValue="${paramMap.officeName}"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <tr>
        <td><a href="javascript:"
               onclick="$('#categoryId').val('${stats.categoryParentId}');$('#categoryName').val('${stats.categoryParentName}');$('#searchForm').submit();return false;">${stats.categoryParentName}</a>
        </td>
        <td><a href="javascript:"
               onclick="$('#categoryId').val('${stats.categoryId}');$('#categoryName').val('${stats.categoryName}');$('#searchForm').submit();return false;">${stats.categoryName}</a>
        </td>
        <td>${stats.cnt}</td>
        <td>${stats.hits}</td>
        <td><fmt:formatDate value="${stats.updateDate}" type="both"/></td>
        <td><a href="javascript:"
               onclick="$('#officeId').val('${stats.officeId}');$('#officeName').val('${stats.officeName}');$('#searchForm').submit();return false;">${stats.officeName}</a>
        </td>
    </tr>
    </c:forEach>
    </tbody>
