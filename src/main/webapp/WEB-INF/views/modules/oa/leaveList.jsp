...
<form:form id="searchForm" modelAttribute="leave" action="${ctx}/oa/leave/list" method="post"
    ...
    <div>
        <label>请假编号：</label><form:input path="ids" htmlEscape="false" maxlength="50" class="input-small"
                                        placeholder="逗号或空格隔开"/>
        <label>创建开始：</label>
        <input id="createDateStart" name="createDateStart" type="text" readonly="readonly" maxlength="20"
               class="input-small Wdate" value="<fmt:formatDate value="${leave.createDateStart}" pattern="yyyy-MM-dd"/>"
               onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
        <label>创建结束：</label>
        <input id="createDateEnd" name="createDateEnd" type="text" readonly="readonly" maxlength="20"
               class="input-small Wdate" value="<fmt:formatDate value="${leave.createDateEnd}" pattern="yyyy-MM-dd"/>"
               onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
        ...
        <tr>
            ...
            <td>
                <c:if test="${leave.processStatus ne '已完成' }">
                    <a href="${ctx}/sys/workflow/processMap?processInstanceId=${leave.processInstanceId}"
                       class="fancybox" data-fancybox-type="iframe">跟踪</a>
                </c:if>
            </td>
        </tr>
    </c:forEach>
