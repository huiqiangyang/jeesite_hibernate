...
<c:if test="${leave.audit}">
    <c:if test="${leave.processStatus eq '部门领导审批'}">
        <form:form id="inputForm" modelAttribute="leave" action="${ctx}/oa/leave/deptLeaderAudit" method="post"
            ...
            <div class="form-actions">
                ...
                <div class="pull-right">
                    <input class="btn btn-warning" type="button" value="不通过" onclick="auditPass(false); "
    ...
    <c:if test="${leave.processStatus eq '调整申请'}">
        <form:form id="inputForm" modelAttribute="leave" action="${ctx}/oa/leave/modifyApply" method="post"
                   class="form-horizontal">
            <form:hidden path="id"/>
            <form:hidden path="pass"/>
    ...
    <c:if test="${leave.processStatus eq '人事审批'}">
        <form:form id="inputForm" modelAttribute="leave" action="${ctx}/oa/leave/hrAudit" method="post"
                   class="form-horizontal">
            <form:hidden path="id"/>
            <form:hidden path="pass"/>
            <div class="control-group">
    ...
    <c:if test="${leave.processStatus eq '销假'}">
        <form:form id="inputForm" modelAttribute="leave" action="${ctx}/oa/leave/reportBack" method="post"
                   class="form-horizontal">
            <form:hidden path="id"/>
            <form:hidden path="pass"/>
            <div class="control-group">
                ...
                <div class="controls">
                    <input id="realityStartTime" name="realityStartTime" type="text" readonly="readonly" maxlength="20"
                           class="Wdate required"
                           value="<fmt:formatDate value="${leave.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
                           onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
                </div>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input id="realityEndTime" name="realityEndTime" type="text" readonly="readonly" maxlength="20"
                           class="Wdate required"
                           value="<fmt:formatDate value="${leave.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
                           onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
                </div>
